...
function renderStats(emails, state) {
  ...
  $("stats").innerHTML = items.map(([label, n]) => `<div class="stat"><b>${n}</b>${label}</div>`).join("");
  $("syncMeta").textContent = state?.lastSuccessfulRun ? `Last successful sync: ${fmt(state.lastSuccessfulRun)}` : "No sync yet";
}
function matches(e, q, status, type) {
  if (status && e.status !== status) return false;
  if (type && e.emailType !== type) return false;
  if (!q) return true;
  const hay = [e.bookCode, e.status, e.subject, e.summary, e.emailType].join(" ").toLowerCase();
...
function renderList(emails) {
  ...
  $("list").innerHTML = rows.map((e) => {
    ...
    return `<article class="card ${cls}">
        <div class="top">
          <div>
            <span class="badge">${e.status || "N/A"}</span>
            <span class="badge">${e.emailType || "Other"}</span>
            <span class="badge">${e.bookCode || "N/A"}</span>
          </div>
...
async function boot() {
  const [emailsRes, stateRes] = await Promise.all([fetch("data/emails.json"), fetch("data/state.json")]);
...
boot().catch((err) => {
  $("syncMeta").textContent = "Failed to load data/emails.json";
